...
function publicEmailError(status, detail) {
  const text = String(detail || '')
  if (
    status === 403 &&
    /verify a domain|own email address|testing emails/i.test(text)
  ) {
    return 'Email is not live yet. Verify notype-mgmt.com in Resend, then set BOOKING_FROM_EMAIL to an address on that domain.'
  }
...
export async function sendBookingEmail({ subject, text, replyTo }) {
  ...
  if (resendKey) {
    ...
    if (!response.ok) {
      ...
      console.warn('[booking-email]', response.status, detail)
      return {
        ok: false,
        error: publicEmailError(response.status, detail),
      }
    }
    return { ok: true }
  }

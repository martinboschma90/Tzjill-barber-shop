...
import { artistHasLiveVideos, artistHasVideos } from '@/cms/artistVideos'
    ...
    <>
      {sections.map((section) => {
        ...
        switch (section.id) {
          ...
          case 'video': {
            const hasRow = previewMode
              ? artistHasVideos(artist)
              : artistHasLiveVideos(artist)
            if (!hasRow) return null
            return (
              <NearMount key="video" minHeight={520} rootMargin="900px">
                <LazySection>
                  <ArtistVideoSlide
                    artist={artist}

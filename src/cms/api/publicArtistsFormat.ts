...
/** Public artist page payload — posters + clips only, no original files. */
export function slimPublicArtistDetail(artist: Artist): Artist {
  const videos = (artist.videos ?? []).map((video) => ({
    ...video,
    videoUrl: '',
    clipUrl: video.clipUrl?.trim() || undefined,
    posterUrl: video.posterUrl?.trim() || undefined,
  }))
  return {
    ...artist,
    videoUrl: undefined,

...
export function normalizeArtistVideos(
  ...
  const fromCollection = (Array.isArray(artist.videos) ? artist.videos : [])
    .filter((v) => Boolean(v?.videoUrl?.trim() || v?.clipUrl?.trim() || v?.posterUrl?.trim()))
    .filter((v) => {
      const key = v.clipUrl?.trim() || v.videoUrl?.trim() || v.id
...
export function artistHasVideos(
  ...
  if (
    videos.some(
      (v) =>
        Boolean(v?.clipUrl?.trim()) ||
        Boolean(v?.posterUrl?.trim()) ||
...
/** Public Shows row — clip or poster only, never the original file. */
export function artistHasLiveVideos(artist: Pick<Artist, 'videos'>): boolean {
  return (Array.isArray(artist.videos) ? artist.videos : []).some(
    (v) => Boolean(v?.clipUrl?.trim()) || Boolean(v?.posterUrl?.trim()),
  )
}
    ...
    const clipUrlRaw =
      typeof row.clipUrl === 'string'
        ...
        : typeof row.clip_url === 'string'
          ? row.clip_url
          : ''
    if (!videoUrl.trim() && !clipUrlRaw.trim()) continue

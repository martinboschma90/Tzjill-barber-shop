...
export function TrafficDashboard({
  ...
  return (
    <>
      <section className="cms-traffic-panel mb-5 overflow-hidden rounded-2xl border border-neutral-200 bg-white text-neutral-900">
        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-neutral-200 px-4 py-2.5">
          <div className="flex items-center gap-2.5">
            ...
            <h3 className="text-sm font-semibold tracking-tight text-neutral-900">Website traffic</h3>
          </div>
          <div className="flex items-center gap-1 rounded-lg bg-neutral-50 p-0.5">
            {periods.map((period) => (
              <button
                key={period}
                type="button"
        ...
        {!loading && error ? (
          <div className="flex items-center justify-between gap-3 px-4 py-4">
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-lg border border-neutral-200 px-2.5 py-1.5 text-[11px] font-medium text-neutral-700 hover:bg-neutral-50"
            >
        ...
        {!loading && !error && data ? (
          <>
            <div className="grid gap-3 border-b border-neutral-200 px-4 py-3 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
            ...
            <div className="grid gap-3 border-b border-neutral-200 p-3 lg:grid-cols-[minmax(0,1.65fr)_minmax(220px,0.9fr)]">
              <div className="min-h-[240px] lg:min-h-[300px]">
                {mapDots.length ? (
                  <WorldTrafficMap dots={mapDots} />
                ) : (
                  <div className="flex h-full min-h-[240px] items-center justify-center rounded-xl bg-neutral-50 text-[11px] text-neutral-400">
                    Nog geen locaties
                  </div>
                )}
              </div>
      ...
      {!loading && !error && data ? (
        <>
          ...
          >
            <div className="overflow-hidden rounded-2xl border border-neutral-200 bg-white text-neutral-900">
      ...
      {!loading && !error && data ? (
        <>
          ...
          >
            <div className="grid gap-4 rounded-2xl border border-neutral-200 bg-white p-4 text-neutral-900 lg:grid-cols-[160px_minmax(0,1fr)]">
              <div className="flex flex-col items-center justify-center">
                <Donut newShare={newShare} returningShare={returningShare} />
                <div className="mt-3 space-y-1 text-center text-[11px] text-neutral-500">
                  <p>
                    <span className="mr-1.5 inline-block h-2 w-2 rounded-full bg-emerald-400" />
                    Nieuw {Math.round(newShare * 100)}%
                  </p>
                  <p>
                    <span className="mr-1.5 inline-block h-2 w-2 rounded-full bg-neutral-200" />
                  ...
                  return (
                    <li key={key}>
                      <div className="mb-1 flex items-center justify-between text-[12px]">
                        <span className="font-medium text-neutral-800">{channelLabels[key]}</span>
...
function InsightCard({
  ...
  return (
    <div className="rounded-xl border border-neutral-200 bg-white px-3 py-3 text-neutral-900">
      <div className="flex items-start justify-between gap-2">
        <p className="text-[10px] font-semibold tracking-wider text-neutral-400 uppercase">{label}</p>
        <span className="inline-flex h-7 w-7 items-center justify-center rounded-md bg-neutral-100 text-neutral-500">
          {icon}
        </span>
      </div>
...
function Metric({
  ...
  return (
    <div className="relative overflow-hidden rounded-lg bg-neutral-50 px-3 py-2">
        ...
        {hideDelta ? null : (
          <span
            className={`mb-1 inline-flex items-center gap-0.5 text-[11px] font-medium ${
              positive ? 'text-emerald-600' : 'text-red-400'
            }`}
          >
            {positive ? (
...
function RankedList({
  ...
  return (
    <div className="border-t border-neutral-200 p-3 xl:border-t-0 xl:border-l xl:first:border-l-0">
      <p className="mb-2 flex items-center gap-1.5 text-[10px] font-semibold tracking-wider text-neutral-500 uppercase">
        {icon}
        {title}
      </p>
      {list.length === 0 ? (
        <p className="text-[11px] text-neutral-400">Nog geen gegevens</p>
      ) : (
            ...
            return (
              <li key={`${label(row)}-${index}`}>
                <div className="mb-0.5 flex items-center gap-2 text-[11px]">
                  <span className="w-3 text-neutral-400">{index + 1}</span>
...
function CompactPlaces({
  ...
  return (
    <div className="rounded-xl border border-neutral-200 bg-neutral-50 px-3 py-2.5">
      ...
      {rows.length === 0 ? (
        <p className="text-[11px] text-neutral-400">{emptyHint || 'Nog geen gegevens'}</p>
      ) : (
        <ol className="space-y-1.5">
          {rows.map((row) => {
            ...
            return (
              <li key={row.key}>
                <div className="mb-0.5 flex items-center gap-1.5 text-[11px]">
                  ...
                  <span className="tabular-nums text-neutral-500">{number(row.value)}</span>
                </div>
...
function Donut({
  ...
  return (
    <svg viewBox="0 0 100 100" className="h-24 w-24" aria-hidden>
      <circle cx="50" cy="50" r={r} fill="none" stroke="#e5e5e5" strokeWidth="12" />
      <circle
        cx="50"
        cy="50"
        r={r}

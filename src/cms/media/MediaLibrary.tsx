...
type MediaLibraryProps = {
  ...
  compact?: boolean
  /** Picker starts in Live (site files) unless choosing a raw source video. */
  folder?: MediaFolder
}
...
export function MediaLibrary({
  ...
  const [folder, setFolder] = useState<MediaFolder>(folderProp ?? 'live')
  ...
  const folderAssets = useMemo(() => {
    const byKind = assets.filter(
      (asset) => selectKind === 'any' || asset.kind === selectKind,
    )
    return byKind.filter((asset) =>
      folder === 'live' ? isLiveSiteAsset(asset) : isSourceVideoAsset(asset),
    )
  }, [assets, folder, selectKind])

  const visible = useMemo(() => {
    const normalizedQuery = query.trim().toLocaleLowerCase('nl')
    const result = folderAssets.filter((asset) => {
  ...
  }, [folderAssets, filter, query, sort])

  const liveAssets = useMemo(
    () =>
      assets.filter(
        (asset) =>
          (selectKind === 'any' || asset.kind === selectKind) &&
          isLiveSiteAsset(asset),
      ),
    [assets, selectKind],
  )
  const sourceAssets = useMemo(
    () =>
      assets.filter(
        (asset) =>
          (selectKind === 'any' || asset.kind === selectKind) &&
          isSourceVideoAsset(asset),
      ),
    [assets, selectKind],
  )

  const imageCount = folderAssets.filter((asset) => asset.kind === 'image').length
  const videoCount = folderAssets.filter((asset) => asset.kind === 'video').length
  const localCount = folderAssets.filter((asset) => !asset.publicUrl).length
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="relative min-w-[12rem] flex-1">
            ...
            <input
              ...
              placeholder={folder === 'live' ? 'Zoek in Live…' : 'Zoek originelen…'}
      ...
      <div className="flex min-h-[64vh]">
        <aside className="cms-media-sidebar hidden w-52 shrink-0 border-r border-neutral-200 bg-neutral-50 p-4 lg:block">
          ...
          <MediaNavButton
            ...
            onClick={() => {
              setFolder('live')
              setFilter(selectKind === 'any' ? 'all' : selectKind)
            }}
          />
          <MediaNavButton
            active={folder === 'sources'}
          ...
          {folder === 'live' ? (
            <>
              <p className="mb-3 mt-6 px-2 text-[10px] font-semibold uppercase tracking-[0.1em] text-neutral-400">
                Kiezen
              </p>
              {selectKind !== 'video' ? (
                <MediaNavButton
          ...
          <div className="mt-8 border-t border-neutral-200 pt-5">
            ...
            <div className="flex items-center justify-between gap-2 px-2 py-1.5 text-xs text-neutral-600">
              ...
              <span className="tabular-nums">{folderAssets.length - localCount}</span>
            </div>
            {localCount > 0 ? (
        ...
        <div className="min-w-0 flex-1">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-neutral-200 px-4 py-3 sm:px-5">
            <div>
              <p className="text-sm font-semibold text-neutral-900">
                {folder === 'live' ? 'Live' : 'Originelen'}
              </p>
              <p className="mt-0.5 text-[11px] text-neutral-500">
                {folder === 'live'
                  ? 'Alleen wat de website laadt: foto’s, posters en korte clips.'
                  : 'Ruwe video’s om in het CMS een clip van te maken. Niet op de site.'}
                {ready ? ` · ${folderAssets.length} bestanden` : ' · Media laden…'}
          ...
          <div className="flex flex-wrap items-center gap-2 border-b border-neutral-200 px-4 py-3 sm:px-5">
            ...
            <div className="flex items-center gap-1 lg:hidden">
              ...
              {folder === 'live' && selectKind !== 'video' ? (
                <FilterButton active={filter === 'image'} onClick={() => setFilter('image')}>
                  Foto’s
                </FilterButton>
              ) : null}
              {folder === 'live' && selectKind !== 'image' ? (
                <FilterButton active={filter === 'video'} onClick={() => setFilter('video')}>
                  Video’s
                </FilterButton>
              ) : null}
          ...
          <article className="group overflow-hidden rounded-xl border border-neutral-200 bg-white transition-shadow hover:shadow-md">
            ...
            >
              ...
              <span className="absolute left-2 top-2 inline-flex items-center gap-1 rounded-md bg-black/70 px-1.5 py-1 text-[9px] font-medium text-white backdrop-blur">
                ...
                {liveAssetLabel(asset)}
              </span>
              {!asset.publicUrl ? (
                <span className="absolute right-2 top-2 inline-flex items-center gap-1 rounded-md bg-amber-600 px-1.5 py-1 text-[9px] font-medium text-white">
                  Alleen hier
                </span>
              ) : null}
            </button>
